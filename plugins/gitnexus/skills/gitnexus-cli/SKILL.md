...
This plugin expects the GitNexus CLI to be available on PATH. Install it globally before enabling the MCP server:

```bash
npm install -g gitnexus
```

`npx gitnexus analyze` also works without a global install, but the plugin MCP configuration starts `gitnexus mcp` directly for faster startup and fewer network/cache surprises.
